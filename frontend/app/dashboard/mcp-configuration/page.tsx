...
import { Edit3, Eye, EyeOff, Plus, Save, Settings, Trash2 } from "lucide-react";
...
export default function McpConfigurationPage() {
  ...
  const {
    data: serverDefinitions = [],
    isLoading: definitionsLoading,
    error: serverDefinitionsError,
  } = useMcpServerDefinitionsQuery();
  const {
  ...
  const [selectedServerSlug, setSelectedServerSlug] = useState<string | null>(
    null,
  );
  ...
  const [editedConfigValues, setEditedConfigValues] = useState<
    Record<string, string>
  >({});

  const selectedServerDefinition = useMemo(() => {
    if (!selectedServerSlug) return undefined;
    return serverDefinitions.find((definition) =>
      definition.slug === selectedServerSlug
    );
  ...
  const handleAddServer = () => {
    ...
    const displayName = newDisplayName.trim() ||
      `${selectedServerDefinition.displayName} ${existingCount + 1}`;

    createMutation.mutate({
      serverId: selectedServerDefinition.id,
      displayName,
      configValues: buildConfigPayload(
        selectedServerDefinition.configFields,
        newServerConfig,
      ),
    });
  };
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mb-6">
          <Button
            ...
            className="gap-2"
            disabled={showAddForm || showLoadingState}
          >
          ...
          ? (
            <div className="space-y-4">
              {userConfigurations.map((configuration) => {
                const statusClass = statusColours[configuration.status] ??
                  statusColours.inactive;
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            onClick={() => (isEditing
                              ? handleCancelEditing()
                              : startEditingConfiguration(configuration))}
                          >
                            {isEditing
                              ? <EyeOff className="h-4 w-4" />
                        ...
                        <CardContent>
                          <div className="space-y-4">
                            ...
                            <div className="flex gap-3">
                              <Button
                                onClick={() =>
                                  handleUpdateConfiguration(configuration)}
                                disabled={updateMutation.isPending}
                              >
                                <Save className="h-4 w-4 mr-2" />
            ...
            <Card>
              <CardContent className="text-center py-12">
                ...
                <p className="text-white/60 mb-4">
                  Get started by adding your first Model Context Protocol
                  server.
                </p>
